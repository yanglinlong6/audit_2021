...
def loadFinnanceExcelToMysql(fileName, sheetName):
    ...
    '''将Excel表数据导入mysql'''
    conn_updata = pymysql.Connect(host='192.168.5.20', port=3306, user='biz_glsx_data',
                             passwd='unEKczDc', db='glsx_biz_data', charset='utf8')

    #pd.set_option('display.max_columns', 40)
    data = pd.read_excel(fileName, sheet_name=sheetName, header=0)
    newData = data[['单据类型', '单据编号', '业务日期', '客户', '渠道', '物料编码', '物料名称', '产品类型', '设备类型', '计价数量',
                '含税单价', '单价', '收入', '业务类型', '分摊期限']]
    '''
    ...
    for idx in newData.index:
        ...
        value = (newData.loc[idx, '单据类型'], newData.loc[idx, '单据编号'], newData.loc[idx, '业务日期'], newData.loc[idx, '客户'],
                 newData.loc[idx, '渠道'], newData.loc[idx, '物料编码'], newData.loc[idx, '物料名称'], newData.loc[idx, '产品类型'],
                 newData.loc[idx, '设备类型'], newData.loc[idx, '计价数量'], hwPriceTax, hwPrice,
                 newData.loc[idx, '收入'], newData.loc[idx, '业务类型'], newData.loc[idx, '分摊期限'])
        # print(value)
        valueArr.append(value)

    cs = conn_updata.cursor()
    sql = "insert into " + tableName + "(order_type,order_no,finance_time,cus_name,sale_type,product_code," \
...
def loadDjExcelToMysql(fileName, sheetName):
    ...
    data = pd.read_excel(fileName, sheet_name=sheetName, header=0)
    newData = data[['单据类型', '单据编号', '业务日期', '客户', '渠道', '物料编码', '物料名称', '产品类型', '设备类型', '计价数量',
                '含税单价', '单价', '硬件（总）（不含税）', '硬件税率', '服务（不含税）', '服务税率', '服务套餐', '收入', '业务类型',
                    '分摊期限', '安装', '安装税率']]
    newData['含税单价'].fillna(0, inplace=True)
    newData['单价'].fillna(0, inplace=True)
    newData['收入'].fillna(0, inplace=True)
    ...
    for idx in newData.index:
        ...
        value = (newData.loc[idx, '单据类型'], newData.loc[idx, '单据编号'], newData.loc[idx, '业务日期'], client,
                 newData.loc[idx, '渠道'], newData.loc[idx, '物料编码'], newData.loc[idx, '物料名称'], newData.loc[idx, '产品类型'],
                 newData.loc[idx, '设备类型'], newData.loc[idx, '计价数量'], hwPriceTax, hwPrice, pkPrice, pkPriceRate,
                 newData.loc[idx, '服务套餐'],
                 newData.loc[idx, '收入'], newData.loc[idx, '业务类型'], newData.loc[idx, '分摊期限'],
                 newData.loc[idx, '分摊期限'], newData.loc[idx, '安装'], newData.loc[idx, '安装税率'])
        print(value)
...
def updateCustomerChannel():
    '''更新客户的渠道'''
    dbConn = pymysql.Connect(host='192.168.5.20', port=3306, user='biz_glsx_data',
                             passwd='unEKczDc', db='glsx_biz_data', charset='utf8')
...
if __name__ == "__main__":
    ...
    filePath = "C:\\Users\\333\\Desktop\\2022审计\\应收拆分数据-2022年12月\\"

    # fileName = filePath + "【数据源】-应收单12月.xlsx"
    # sheetName = "Sheet1"
    # loadFinnanceExcelToMysql(fileName, sheetName)


    # E盾
    # fileName = filePath + "1.拆分数据-虎哥E盾结算单与明细数据12月-陈冰恋.xlsx"
    # sheetName = '3.虎哥E盾--渠道应收单原表'
    # loadDjExcelToMysql(fileName, sheetName)
    # # 渠道嘀加
    # fileName = filePath + "3.拆分数据-渠道2022年12月滴加数据-柳云飞.xlsx"
    # sheetName = 'Sheet2'
    # loadDjExcelToMysql(fileName, sheetName)
    # # # 直销
    # fileName = filePath + "4.拆分数据-广汇2022年12月-廖敏玲.xlsx"
    # sheetName = '廖敏玲'
    # loadDjExcelToMysql(fileName, sheetName)
